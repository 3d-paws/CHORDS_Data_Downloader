...
def wind_direction_mapper(wind_dir:int , fill_empty) -> str:
    ...
    for i in range(len(wind_val_lookup)-1):
        if wind_dir == -999.99 or wind_dir == -999:
            return -999.99
        elif wind_dir < 0 or wind_dir > 360:
            return fill_empty
        elif wind_dir >= wind_val_lookup[i] and wind_dir <= wind_val_lookup[(i+1)]:
...
def is_wind_dir(measurement:str) -> bool:
    ...
    wind_dir_shortnames = [ # as new shortnames get added to database, this must be updated
        'wd', 'wgd', 'wind_direction', 'wind_dir'
    ]
...
"""
def get_columns(dictionary_list:list, portal_name:str) -> list:
...
"""
def build_headers(measurements:list, columns_desired:list, portal_name:str) -> list:
...
def struct_has_data(measurements:np.ndarray, time:np.ndarray) -> bool:
    ...
    if (len(measurements) == 0) or (len(time) == 0):
        return False
